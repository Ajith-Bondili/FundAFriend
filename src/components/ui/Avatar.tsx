...
type AvatarImageProps = Omit<React.ComponentProps<typeof Image>, 'src' | 'alt'> & { src: string; alt?: string };
const AvatarImage = React.forwardRef<HTMLImageElement, AvatarImageProps>(({ className, src, alt = "Avatar", ...props }, ref) => (
  <Image ref={ref as any} src={src} alt={alt} className={cn("aspect-square h-full w-full object-cover", className)} width={40} height={40} {...props} />
));
